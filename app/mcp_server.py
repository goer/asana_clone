...
def list_workspaces(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[WorkspaceResponse]:
    """List all workspaces for the current user."""
    workspace_rows = db.scalars(
        select(Workspace).join(UserWorkspace, Workspace.id == UserWorkspace.workspace_id)
        .filter(UserWorkspace.user_id == current_user.id).order_by(Workspace.created_at)
    ).all()
...
def create_workspace(payload: WorkspaceCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> WorkspaceResponse:
...
def get_workspace(workspace_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> WorkspaceResponse:
...
def update_workspace(workspace_id: int, payload: WorkspaceUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> WorkspaceResponse:
    """Update a workspace."""
    workspace = db.get(Workspace, workspace_id)
    if not workspace:
        raise HTTPException(status_code=404, detail="Workspace not found")
...
def delete_workspace(workspace_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
    """Delete a workspace."""
    workspace = db.get(Workspace, workspace_id)
    if not workspace:
        raise HTTPException(status_code=404, detail="Workspace not found")
...
def list_projects(workspace_id: int = Query(..., description="Workspace identifier"), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[ProjectResponse]:
...
def create_project(payload: ProjectCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> ProjectResponse:
...
def update_project(project_id: int, payload: ProjectUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> ProjectResponse:
    """Update a project."""
    project = db.get(Project, project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
...
def create_task(payload: TaskCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> TaskResponse:
    """Create a new task."""
...
def update_task(task_id: int, payload: TaskUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> TaskResponse:
    """Update a task."""
    task = db.get(Task, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
...
def list_sections(project_id: int = Query(..., description="Project identifier"), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[SectionResponse]:
...
def create_section(payload: SectionCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> SectionResponse:
...
def update_section(section_id: int, payload: SectionUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> SectionResponse:
    """Update a section."""
...
def delete_section(section_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
    """Delete a section."""
    section = db.get(Section, section_id)
    if not section:
        raise HTTPException(status_code=404, detail="Section not found")
...
def list_comments(task_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[CommentResponse]:
...
def create_comment(task_id: int, payload: CommentCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> CommentResponse:
...
def update_comment(comment_id: int, payload: CommentUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> CommentResponse:
    """Update a comment."""
...
def delete_comment(comment_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
    """Delete a comment."""
    comment = db.get(Comment, comment_id)
    if not comment:
        raise HTTPException(status_code=404, detail="Comment not found")
...
def list_tags(workspace_id: int = Query(..., description="Workspace identifier"), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[TagResponse]:
...
def create_tag(payload: TagCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> TagResponse:
    """Create a new tag."""
    membership = db.get(UserWorkspace, (current_user.id, payload.workspace_id))
    if not membership:
        raise HTTPException(status_code=403, detail="Not a member of workspace")
...
def update_tag(tag_id: int, payload: TagUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> TagResponse:
    """Update a tag."""
    tag = db.get(Tag, tag_id)
    if not tag:
        raise HTTPException(status_code=404, detail="Tag not found")
    membership = db.get(UserWorkspace, (current_user.id, tag.workspace_id))
    if not membership:
        raise HTTPException(status_code=403, detail="Not a member of workspace")
    for field, value in payload.model_dump(exclude_none=True).items():
...
def add_tag_to_task(task_id: int, tag_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
    """Add a tag to a task."""
    task = db.get(Task, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
...
def remove_tag_from_task(task_id: int, tag_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
...
def list_teams(workspace_id: int = Query(..., description="Workspace identifier"), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[TeamResponse]:
...
def create_team(payload: TeamCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> TeamResponse:
    """Create a new team."""
    membership = db.get(UserWorkspace, (current_user.id, payload.workspace_id))
    if not membership:
        raise HTTPException(status_code=403, detail="Not a member of workspace")
    ...
    db.add(team)
    db.flush()
    db.add(UserTeam(user_id=current_user.id, team_id=team.id))
    db.commit()
    db.refresh(team)
...
def add_team_member(team_id: int, user_id: int = Query(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
    """Add a member to a team."""
    team = db.get(Team, team_id)
    if not team:
        raise HTTPException(status_code=404, detail="Team not found")
    membership = db.get(UserWorkspace, (current_user.id, team.workspace_id))
    if not membership:
        raise HTTPException(status_code=403, detail="Not a member of workspace")
...
def remove_team_member(team_id: int, user_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
...
def list_attachments(task_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[AttachmentResponse]:
...
def create_attachment(task_id: int, payload: AttachmentCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> AttachmentResponse:
    ...
    if not membership:
        raise HTTPException(status_code=403, detail="Not a member of workspace")
    attachment = Attachment(**payload.model_dump(), task_id=task_id, uploader_id=current_user.id)
    db.add(attachment)
    db.commit()
...
def delete_attachment(attachment_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
...
def list_custom_fields(project_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[CustomFieldResponse]:
...
def create_custom_field(project_id: int, payload: CustomFieldCreateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> CustomFieldResponse:
...
def delete_custom_field(field_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
...
def set_custom_field_value(task_id: int, field_id: int, payload: CustomFieldValueRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
...
def clear_custom_field_value(task_id: int, field_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> None:
